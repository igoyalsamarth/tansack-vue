...
const columns = [
    ...
    columnHelper.accessor('pendingApprovalFrom', {
        ...
        cell: info => info.getValue() === 'no' ? 'No Approvals' : info.getValue() === 'all' ? 'All Approved' : info.getValue() === 'pending' ? 'Pending Submission' : info.getValue() === null ? '-' : info.getValue()
    }),
    columnHelper.accessor('paymentStatus', {
        header: 'Payment Status',
        cell: info => info.getValue() === 'paid' ? 'Paid' : info.getValue() === 'partial' ? 'Partially Paid' : info.getValue() === 'unpaid' ? 'Unpaid' : '-'
    }),
...
<template>
    <div class="w-full">
                ...
                <tr v-for="headerGroup in table.getHeaderGroups()" :key="headerGroup.id">
                    <td :class="{
                        ...
                        <div class="flex flex-col gap-2 font-medium whitespace-nowrap" :class="{
                            ...
                            <template
                                v-if="!header.isPlaceholder && header.column.getCanFilter() && header.id !== 'approval' && header.id !== 'amount'">
            ...
            <tbody>
                <tr class="border border-[#F4F4F5] font-medium text-sm text-[#52525B]"
                    ...
                    <td class="py-4 px-2 text-[#1D4ED8]">
                        <FlexRender v-if="row.getVisibleCells()[2].getValue() !== null"
                            :render="row.getVisibleCells()[2].column.columnDef.cell"
                            :props="row.getVisibleCells()[2].getContext()" />
                        <p v-else>-</p>
                    </td>
                    <td class="py-4 px-2">
                        <FlexRender :render="row.getVisibleCells()[3].column.columnDef.cell"
                            :props="row.getVisibleCells()[3].getContext()" />
                    </td>
                    <td class="py-4 px-2">
                        <FlexRender :render="row.getVisibleCells()[4].column.columnDef.cell"
                            :props="row.getVisibleCells()[4].getContext()" />
                    </td>
                    <td class="py-4 px-2">
                        <FlexRender :render="row.getVisibleCells()[5].column.columnDef.cell"
                            :props="row.getVisibleCells()[5].getContext()" />
                    </td>
                    <td class="py-4 px-2">
                        <FlexRender :render="row.getVisibleCells()[6].column.columnDef.cell"
                            :props="row.getVisibleCells()[6].getContext()" />
                    </td>
                    <td class="py-4 px-2">
                        <FlexRender :render="row.getVisibleCells()[7].column.columnDef.cell"
        ...
        <div class="flex justify-between items-center py-3 bg-[#F9FAFB]">
            <div class="px-3">
                <p class="text-sm font-medium text-[#6B7280]">{{ table.getRowCount() }} Expense Items</p>
            </div>
            <div class="flex items-center gap-2 px-3">
                <div class="flex gap-7">
                    ...
                    <select
                        ...
                        :value="table.getState().pagination.pageSize" @change="handlePageSizeChange">
                ...
                <div class="flex gap-1">
                    <button class="border rounded px-2 py-[6px] border-[#D1D5DB] text-xs font-medium text-[#6B7280]"
                        @click="() => table.setPageIndex(0)" :disabled="!table.getCanPreviousPage()">
                        First
                    </button>
                    <button class="border rounded p-2 border-[#D1D5DB] text-xs font-medium text-[#6B7280]"
                    ...
                    <button v-for="page in visiblePages" :key="page"
                        ...
                        @click="() => table.setPageIndex(Number(page) - 1)" :class="{
                            'bg-[#2563EB] border border-[#1D4ED8] text-white': table.getState().pagination.pageIndex === Number(page) - 1,
                            'py-1 h-fit': page === '...'
                        }" :disabled="page === '...'">
                        {{ page }}
                    </button>
                    <button class="border rounded px-2 border-[#D1D5DB] text-xs font-medium text-[#6B7280]"
